...
public:
    ...
    int CheckGlobalRouteOListSize(boost::shared_ptr<const NetworkAgentMock> agent,
        ...
        const NetworkAgentMock::McastRouteEntry *ermvpn_rt =
            agent->McastRouteLookup("mvpn_fabric", prefix);
        if (ermvpn_rt == NULL)
            return false;
    ...
    bool CheckOListElem(boost::shared_ptr<const NetworkAgentMock> agent,
        ...
        const string &address, const string &encap, const string source_address,
        ...
        const NetworkAgentMock::McastRouteEntry *ermvpn_rt =
            other_agent->McastRouteLookup("mvpn_fabric", prefix);
        if (ermvpn_rt == NULL)
            return false;
        ...
        if (!(ermvpn_rt->entry.nlri.source_address == source_address))
            return false;
        int label = GetLabel(other_agent, "mvpn_fabric", prefix);
        if (label == 0)
            return false;
...
<config>\
    ...
    <virtual-network name='mvpn_fabric'>\
        <network-id>1000</network-id>\
    </virtual-network>\
    <routing-instance name='mvpn_fabric'>\
...
public:
    ...
    virtual void SetUp() {
        ...
        fabric_ermvpn_ = static_cast<ErmVpnTable *>(
            bs_x_->database()->FindTable("mvpn_fabric.ermvpn.0"));
...
TEST_F(BgpMvpnOneControllerTest, Basic) {
    ...
    TASK_UTIL_EXPECT_EQ(4, master_->Size()); // red, blue, gree, mvpn_fabric
    ...
    agent_xa_->AddMcastRoute("mvpn_fabric", mroute, "10.1.1.3", "50-60");
...
protected:
    ...
    virtual void SetUp() {
        ...
        green_ = static_cast<MvpnTable *>(
            bs_x_->database()->FindTable("green.mvpn.0"));
        fabric_ermvpn_ = static_cast<ErmVpnTable *>(
            bs_y_->database()->FindTable("mvpn_fabric.ermvpn.0"));
        red_inet_ = static_cast<InetTable *>(
            bs_x_->database()->FindTable("red.inet.0"));
        green_inet_ = static_cast<InetTable *>(
            bs_x_->database()->FindTable("green.inet.0"));
...
TEST_F(BgpMvpnTwoControllerTest, RedSenderGreenReceiver) {
    ...
    TASK_UTIL_EXPECT_NE(static_cast<MvpnRoute *>(NULL),
                        green_->FindType1ADRoute());
    TASK_UTIL_EXPECT_EQ(4, master_->Size()); // red, blue, gree, mvpn_fabric
    TASK_UTIL_EXPECT_EQ(4, master_y_->Size()); // red, blue, gree, mvpn_fabric
